...
export function Sidebar({ 
  ...
  const menuItems = [
    ...
    { id: 'Produksi Pakan', icon: FlaskConical, label: 'Produksi Pakan' },
      ...
      <aside className={cn(
        ...
        <div className="flex items-center h-16 relative group transition-all duration-500 overflow-hidden w-full">
          <div ref={sidebarLogoContainerRef} className={cn(
            ...
            isCollapsed ? "left-[18px] group-hover:opacity-0 group-hover:scale-75 md:left-[18px]" : "left-6"
          )}>
            ...
            <div className={cn(
              ...
              isCollapsed ? "w-0 md:w-0" : "w-[150px]"
            )}>
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={cn(
                ...
                activeTab === item.id 
                  ? (isCollapsed ? "text-orange-500 md:text-orange-500" : "bg-orange-500 text-white shadow-lg shadow-orange-500/20")
                  : "text-slate-500 hover:text-slate-900 hover:bg-slate-50",
                isCollapsed && "justify-center px-0 md:justify-center md:px-0"
              )}
            >
              <item.icon size={18} className={cn(
                "shrink-0 transition-colors duration-200",
                activeTab === item.id ? (isCollapsed ? "text-orange-500 md:text-orange-500" : "text-white") : "text-slate-400 group-hover:text-slate-600"
              )} />
              
              {/* Label for expanded state */}
              <div className={cn(
                "transition-all duration-500 text-left overflow-hidden",
                isCollapsed ? "w-0 opacity-0 md:w-0 md:opacity-0" : "w-[160px] opacity-100 pl-3"
              )}>
                <span className="text-[13px] font-medium whitespace-nowrap block">
                  {item.label}
                </span>
              </div>
              ...
              {isCollapsed && (
                <div className="hidden md:block absolute left-12 bg-white border border-slate-200 text-slate-900 px-2 py-1 rounded shadow-md opacity-0 group-hover:opacity-100 pointer-events-none z-50 font-bold text-[11px] whitespace-nowrap">
                  {item.label}
                </div>
              )}
        ...
        <div className={cn("mt-auto border-t border-slate-100/60 relative", isCollapsed ? "p-3 md:p-3" : "p-4")}>
          {/* Profile Menu Popup */}
          <div className={cn(
            ...
            isCollapsed ? "left-4 w-48 md:left-4 md:w-48" : "left-4 right-4",
          ...
          <button 
            ...
            className={cn(
              ...
              isCollapsed ? "justify-center p-2 md:justify-center md:p-2" : "p-2"
            )}
          >
            ...
            <div className={cn(
              ...
              isCollapsed ? "w-0 md:w-0" : "w-[160px]"
            )}>
